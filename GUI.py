...
class GUI:
    def __init__(self, img_file, compos_json_file=None, output_dir='data/output'):
        ...
        self.img_resized = cv2.resize(
            self.img, (self.img_reshape[1], self.img_reshape[0]))
        self.file_name = img_file.replace('\\', '/').split('/')[-1][:-4]

        self.output_dir = output_dir
        self.ocr_dir = pjoin(
            self.output_dir, 'ocr') if output_dir is not None else None
        self.non_text_dir = pjoin(
            self.output_dir, 'ip') if output_dir is not None else None
        self.merge_dir = pjoin(
            self.output_dir, 'uied') if output_dir is not None else None
        self.layout_dir = pjoin(
            self.output_dir, 'layout') if output_dir is not None else None
        self.label_studio_dir = pjoin(
            self.output_dir, 'label') if output_dir is not None else None
        ...
        self.detection_result_img = {
            'text': None, 'non-text': None, 'merge': None}

        # visualize group of compos with repetitive layout
        self.layout_result_img_group = None
        self.layout_result_img_pair = None      # visualize paired groups
        # visualize list (paired group) boundary
        self.layout_result_img_list = None
    ...
    def save_layout_result_imgs(self):
        ...
        cv2.imwrite(pjoin(self.layout_dir, self.file_name +
                    '-group.jpg'), self.layout_result_img_group)
        cv2.imwrite(pjoin(self.layout_dir, self.file_name +
                    '-pair.jpg'), self.layout_result_img_pair)
        cv2.imwrite(pjoin(self.layout_dir, self.file_name +
                    '-list.jpg'), self.layout_result_img_list)
    ...
    def save_layout_result_json(self):
        ...
        json.dump(
            js, open(pjoin(self.layout_dir, self.file_name + '.json'), 'w'), indent=4)
    ...
    def save_list(self):
        ...
        json.dump(
            js, open(pjoin(self.layout_dir, self.file_name + '-list.json'), 'w'), indent=4)
    ...
    def save_detection_result(self):
        if not os.path.exists(pjoin(self.merge_dir, self.file_name + '.jpg')):
            ...
            cv2.imwrite(pjoin(self.ocr_dir, self.file_name + '.jpg'),
                        self.detection_result_img['text'])
            cv2.imwrite(pjoin(self.non_text_dir, self.file_name +
                        '.jpg'), self.detection_result_img['non-text'])
            cv2.imwrite(pjoin(self.merge_dir, self.file_name +
                        '.jpg'), self.detection_result_img['merge'])
        if not os.path.exists(pjoin(self.merge_dir, self.file_name + '.json')):
            json.dump(self.compos_json, open(
                pjoin(self.merge_dir, self.file_name + '.json'), 'w'), indent=4)
    ...
    *****************************
    ...
    def detect_element(self, is_ocr=True, is_non_text=True, is_merge=True, img_resize_longest_side=800, show=False):
        ...
        if img_resize_longest_side is not None:
            self.img_reshape = self.resize_by_longest_side(
                img_resize_longest_side)
            self.img_resized = cv2.resize(
                self.img, (self.img_reshape[1], self.img_reshape[0]))
        ...
        if is_ocr:
            self.detection_result_img['text'] = text.text_detection(
                self.img_file, self.ocr_dir, show=show)
        elif os.path.isfile(pjoin(self.ocr_dir, self.file_name + '.jpg')):
            self.detection_result_img['text'] = cv2.imread(
                pjoin(self.ocr_dir, self.file_name + '.jpg'))

        if is_non_text:
            self.detection_result_img['non-text'] = ip.compo_detection(
                self.img_file, self.non_text_dir, key_params, resize_by_height=resize_height, show=show)
        elif os.path.isfile(pjoin(self.non_text_dir, self.file_name + '.jpg')):
            self.detection_result_img['non-text'] = cv2.imread(
                pjoin(self.non_text_dir, self.file_name + '.jpg'))

        if is_merge:
            os.makedirs(self.merge_dir, exist_ok=True)
            compo_path = pjoin(self.non_text_dir, self.file_name + '.json')
            ocr_path = pjoin(self.ocr_dir, self.file_name + '.json')
            self.detection_result_img['merge'], self.compos_json = merge.merge(
                self.img_file, compo_path, ocr_path, self.merge_dir, is_remove_bar=True, is_paragraph=True, show=show)

    def load_detection_result(self):
        '''
        Load json detection result from json file
        '''
        self.compos_json = json.load(
            open(pjoin(self.merge_dir, self.file_name + '.json')))
    ...
    **************************
    ...
    def cvt_compos_json_to_dataframe(self):
        ...
        '''
        self.compos_df = ComposDF(
            json_data=self.compos_json, gui_img=self.img_resized.copy())
    ...
    def recognize_groups(self):
        ...
        # group, alignment_in_group, group_nontext, group_text
        self.compos_df.recognize_element_groups_by_clustering()
    ...
    def cvt_groups_to_list_compos(self):
        ...
        for i in groups:
            if i == -1 or len(groups[i]) == 1:
                continue
            self.lists.append(List(compo_id='l-' + str(list_id), list_class='multi',
        ...
        for i in groups:
            if i == -1 or len(groups[i]) == 1:
                continue
            self.lists.append(List(compo_id='l-' + str(list_id), list_class='single',
                              compo_df=df.loc[groups[i]], list_alignment=df.loc[groups[i][0]]['alignment_in_group']))
            list_id += 1
            # remove selected compos
            df = df.drop(list(groups[i]))
        ...
        for i in range(len(df)):
            ...
            self.compos.append(Compo(
                compo_id='c-' + str(compo_df['id']), compo_class=compo_df['class'], compo_df=compo_df))
    ...
    def recognize_layout(self, is_save=True):
        ...
        print("[Layout Recognition Completed in %.3f s] Input: %s Output: %s" % (time.perf_counter(
        ) - start_time, self.img_file, pjoin(self.layout_dir, self.file_name + '.json')))
        # print(time.ctime(), '\n\n')
    ...
    *********************
    ...
    def get_layout_result_imgs(self):
        self.layout_result_img_group = self.visualize_compos_df(
            'group', show=False)
        self.layout_result_img_pair = self.visualize_compos_df(
            'group_pair', show=False)
        self.layout_result_img_list = self.visualize_lists(show=False)

    def visualize_element_detection(self):
        cv2.imshow('text', cv2.resize(
            self.detection_result_img['text'], (500, 800)))
        cv2.imshow(
            'non-text', cv2.resize(self.detection_result_img['non-text'], (500, 800)))
        cv2.imshow('merge', cv2.resize(
            self.detection_result_img['merge'], (500, 800)))
        cv2.waitKey()
    ...
    def draw_element_detection(self, line=2):
        ...
        colors = {'Text': (0, 0, 255), 'Compo': (
            0, 255, 0), 'Block': (0, 166, 166)}
        for compo in self.compos_json['compos']:
            position = compo['position']
            if compo['class'] == 'Text':
                draw.draw_label(board_text, [position['column_min'], position['row_min'],
                                position['column_max'], position['row_max']], colors[compo['class']], line=line)
            else:
                draw.draw_label(board_nontext, [position['column_min'], position['row_min'],
                                position['column_max'], position['row_max']], colors[compo['class']], line=line)
            draw.draw_label(board_all, [position['column_min'], position['row_min'],
                            position['column_max'], position['row_max']], colors[compo['class']], line=line)
    ...
    def visualize_layout_recognition(self):
        ...
        cv2.imshow('group', cv2.resize(
            self.layout_result_img_group, (500, 800)))
        cv2.imshow('group_pair', cv2.resize(
            self.layout_result_img_pair, (500, 800)))
    ...
    def visualize_container(self, show=True):
        ...
        for i in range(len(containers)):
            ...
            for j in range(len(children)):
                ...
                color = (0, 255, 0) if child['class'] == 'Compo' else (
                    0, 0, 255)
                cv2.rectangle(board, (child['column_min'], child['row_min']),
                              (child['column_max'], child['row_max']), color, 2)
            draw.draw_label(board, (container['column_min'], container['row_min'],
                            container['column_max'], container['row_max']), (166, 166, 0), text='container')
        if show:
